...
import { runnerFor, type StepContext, type StepResult } from './context'
import type { SshKeyResult } from '../../../ports/ssh-keys'
...
export const gitConfig = runnerFor<GitConfig>(async (config, ctx) => {
  ...
  if (pairs.length === 0 && !config.sshKey) return result
  ...
  if (config.sshKey) result.sshKey = await makeKey(config, ctx)

  return result
})

async function makeKey(config: GitConfig, ctx: StepContext): Promise<SshKeyResult> {
  ctx.say('Preparando a chave SSH')

  const outcome = await ctx.ports.sshKeys.ensure(config.email.trim())

  ctx.note(
    outcome === 'created'
      ? `${ctx.program.name}: chave SSH criada em ~/.ssh/id_ed25519 — falta cadastrar a pública no GitHub`
      : outcome === 'already'
        ? `${ctx.program.name}: você já tinha uma chave SSH, não mexi nela`
        : `${ctx.program.name}: não deu para criar a chave SSH`,
    outcome === 'failed' ? 'error' : outcome === 'created' ? 'ok' : 'info',
  )
